...
# Add parent directory to path to allow importing jax_sim
...
class QRSVGLWidget(QtOpenGL.QGLWidget):
    ...
    def paintGL(self):
        ...
        with global_state_mutex:
            global_state_manager.state = copy.deepcopy(self.sim_wrapper.game_state)
            # Fake network stats for interpolation logic
            global_state_manager.state.recv_time = time.time()
            global_state_manager.state.recv_interval = 1/60.0
...
def main():
    ...
    print("Starting RocketSimVis (JAX Backend)...")
    ...
    sim_wrapper = SimWrapper(n_envs=1, max_cars=6)
