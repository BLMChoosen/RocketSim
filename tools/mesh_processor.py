...
Mesh Processor
==============
Builds spatial grid from collision meshes for fast SDF queries.
Generates mesh_data.npz used by the arena SDF.
"""

import numpy as np
import os
from .cmf_loader import load_all_meshes

# Constants
BT_TO_UU = 100.0
CELL_SIZE = 512.0
MAX_TRIS_PER_CELL = 256
...
def build_mesh_grid():
    ...
    tri_mins = np.min(tri_verts, axis=1)
    tri_maxs = np.max(tri_verts, axis=1)
    ...
    # Pre-compute triangle normals and edges
    v0 = tri_verts[:, 0, :]
    v1 = tri_verts[:, 1, :]
    v2 = tri_verts[:, 2, :]
